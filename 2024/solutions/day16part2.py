    ...
    for node in nodes[pos]:
        if node in unvisited:
            ...
            for path in paths[pos]:
                if len(path) == 0:
                    newPaths.append([[pos], newDist])
                elif pos in nodes[path[0][-1]]:
                    newPath = deepcopy(path[0])
                    newPath.append(pos)
                    newPaths.append([newPath, newDist])

            if newDist < distances[node]: 
                distances[node] = newDist

            for path in newPaths:
                if path[1] < newDist: paths[node] = newPaths
                elif path[1] == newDist:
...
finalDist = min(finals)

for dir in dirStrs:
    key = (endPos, dir)
    if key in paths.keys(): 
        for key2 in paths[key]:
            if key2[1] == finalDist: finalPaths.append(key2[0])

alltiles = set()
for path in finalPaths:
    for node in path:
        alltiles.add(node[0])
print(len(alltiles) + 1)

# Laptop did not like this one. It screamed and screamed as it processed all 20,000+ nodes, took like 10 minutes.